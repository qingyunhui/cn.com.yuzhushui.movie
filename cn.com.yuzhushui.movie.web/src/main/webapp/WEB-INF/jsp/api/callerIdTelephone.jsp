...
<body class="mWrap" style="background:#f7f3f3">
	<header class="topHd">
	  ...
	  <div style="position: absolute;top: 3.9em;bottom: 40px;width: 100%;">
		...
		<div class="content" style="height: 8%;"><input name="content" style="width: 100%;height: 100%;" placeholder="输入要查询的手机固话" maxlength="30" size="30" id="content"/></div>
	  </div>
...
<script type="text/javascript">
		var start=false;
		$(function(){
			$("#results").append("<div style='margin: 7px 9px;font-size: 15px;font-family: initial;font-weight: bold;'>请输入要查询的手机固话。</div>");
			$("#content").select();
			$(".btnQuery").click(function(){
				if(start) return false;
				var value=$("#content").val();
				if(value==""){
					layer.msg("要查询的手机固话不能为空.");
					return false;
				}
				$("#results").append("<div style='background: rgba(7, 70, 255, 0.29);font-size: 19px;margin-top: 1px; margin-bottom: 4px;font-family: monospace;'>手机固话:"+value+"</div>");
				$.ajax({
		            type: "POST",
		            url: "${path}api/getCallerIdTelephone.json",
					  data: {tel:value},
					  beforeSend: function() { 
						  $("#content").val("");
						  startTimer();
		              ...
		              success: function(result) {
		            	  ...
		            	  if(datas && datas.success_code==10000){
		            		  ...
		            		  $("#results").append("<div style='background: rgba(101, 255, 7, 0.51);font-size: 16px;margin-top: 4px; margin-bottom: 1px;'>号码所属运营商:"+entity.sp+"</div>");
		            		  $("#results").append("<div style='background: rgba(101, 255, 7, 0.51);font-size: 16px;margin-top: 4px; margin-bottom: 1px;'>号码性质描述:"+entity.rpt_comment+"</div>");
		            	  }else{
